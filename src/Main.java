...
public class Main {
    public static void main(String[] args) {
        ...
        cargarArbolInicial(arbolHabilidades, user1, user2, user3);
        ...
        do {
            ...
            System.out.println("4- Arbol de habilidades");
            ...
            if (opcionPrincipal == 1) {
                ...
                menuConsultas(plataforma, colaJava, colaSistemas, colaLinux, teclado);
            } else if (opcionPrincipal == 4) {
                menuArbol(arbolHabilidades, plataforma, teclado);
            }
    ...
    public static void menuConsultas(Diccionario plataforma, Cola colaJava, Cola colaSistemas, Cola colaLinux, Scanner teclado) {
        int opcionConsultas;
    ...
    public static void cargarArbolInicial(ArbolHabilidades arbolHabilidades, Clase_Perfil user1, Clase_Perfil user2, Clase_Perfil user3) {
        arbolHabilidades.agregarRaiz("Habilidades laborales");
        arbolHabilidades.agregarHabilidad("Habilidades laborales", "Programacion");
        arbolHabilidades.agregarHabilidad("Habilidades laborales", "Sistemas");
        arbolHabilidades.agregarHabilidad("Programacion", "Java");
        arbolHabilidades.agregarHabilidad("Sistemas", "Linux");
        arbolHabilidades.asociarProfesional("Java", user1.getId());
        arbolHabilidades.asociarProfesional("Sistemas", user2.getId());
        arbolHabilidades.asociarProfesional("Linux", user3.getId());
    }

    public static void menuArbol(ArbolHabilidades arbolHabilidades, Diccionario plataforma, Scanner teclado) {
        int opcionArbol;

        do {
            System.out.println("\n  [ SUBMENU 4 - ARBOL DE HABILIDADES ]");
            System.out.println("  1- Agregar habilidad");
            System.out.println("  2- Asociar profesional a habilidad");
            System.out.println("  3- Buscar profesionales por habilidad");
            System.out.println("  4- Mostrar arbol por niveles");
            System.out.println("  5- Mostrar camino a una habilidad");
            System.out.println("  6- Mostrar hojas");
            System.out.println("  7- Volver");
            System.out.print("  Seleccione una opcion: ");
            opcionArbol = teclado.nextInt();
            teclado.nextLine();

            if (opcionArbol == 1) {
                System.out.print("  Ingrese la habilidad padre: ");
                String habilidadPadre = teclado.nextLine();
                System.out.print("  Ingrese la nueva habilidad: ");
                String nuevaHabilidad = teclado.nextLine();
                arbolHabilidades.agregarHabilidad(habilidadPadre, nuevaHabilidad);
            } else if (opcionArbol == 2) {
                System.out.print("  Ingrese la habilidad: ");
                String habilidad = teclado.nextLine();
                System.out.print("  Ingrese el email del profesional: ");
                String email = teclado.nextLine();

                Clase_Perfil perfil = plataforma.recuperar(email);
                if (perfil != null) {
                    arbolHabilidades.asociarProfesional(habilidad, email);
                } else {
                    System.out.println("  Error: no existe un usuario con ese email.");
                }
            } else if (opcionArbol == 3) {
                System.out.print("  Ingrese la habilidad a consultar: ");
                String habilidad = teclado.nextLine();
                arbolHabilidades.buscarProfesionalesPorHabilidad(habilidad);
            } else if (opcionArbol == 4) {
                arbolHabilidades.mostrarPorNiveles();
            } else if (opcionArbol == 5) {
                System.out.print("  Ingrese la habilidad: ");
                String habilidad = teclado.nextLine();
                arbolHabilidades.mostrarCamino(habilidad);
            } else if (opcionArbol == 6) {
                arbolHabilidades.mostrarHojas();
            } else if (opcionArbol != 7) {
                System.out.println("  Opcion invalida.");
            }
        } while (opcionArbol != 7);
    }
